...
function getLoanApplicationDataFromInputs() {
    ...
    /* Controlling error using try catch */
    /* try{
        if(isNaN(month)){
            var e = new Error("Month should be a number", {cause: "coding error"});
            throw e;
        }
    }catch(error){
        ...
        console.log("always execute this after try and catch");
    } */
    
    /* Controlling error by just logging it to the console as an error and let the execution of program continue. Without a try catch */
    ...
    */
    ...
    /* 
        try{
            if(isNaN(month)){
                throw new TypeError("Month should be a number", {cause: "coding error"});
            }else if(month > 12){
                throw new RangeError("Mon should be 1-12");
            }
        }catch(error){
    ...
    */
    ...
    /* 
        try{
            if(isNaN(month)){
                throw new TypeError("Month should be a number", {cause: "user input error"});
            }else if(month > 12){
                throw new RangeError("Mon should be 1-12");
            }
        }catch(error){
            if(error instanceof TypeError){
                ...
                try{
                    if(error.cause === "user input error"){
                       console.log("It's OK. Recoverable error!!");
                    }
                }catch(error){
...
class NumberError extends Error{
    constructor(inputName, number, ...params){
        ...
        //Maintains proper stack trace of where our error was thrown       
